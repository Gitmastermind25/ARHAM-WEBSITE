...
import { WebTeam, DesignTeam, ContentTeam, EventPrTeam, SocialMediaTeam } from "./data";
...
export default function AllTeam() {
  const teams = [
    ...
    { 
        title: "5. Event and Management Team", 
        description: (
            <>
...
<div className="grid grid-cols-2 gap-4 md:gap-8">
  {(team.members || []).filter((member) => !member.large).map((member, idx) => (
    <div
      key={idx}
        ...
        <div className="flex gap-3 mt-1">
          {member.socials?.linkedin && (
            <a
              ...
              className="text-blue-600 text-lg md:text-xl lg:text-2xl"
            >
              <FaLinkedin />
            </a>
          )}
          {member.socials?.instagram && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-pink-600 text-lg md:text-xl lg:text-2xl"
            >
              <FaSquareInstagram />
            </a>
          )}
